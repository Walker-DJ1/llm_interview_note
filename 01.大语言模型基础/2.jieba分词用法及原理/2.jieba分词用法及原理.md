...
- 基于HMM（隐马尔科夫模型）的分词方法：属于由字构词的分词方法，由字构词的分词方法思想并不复杂，它是将分词问题转化为字的分类问题（序列标注问题），规定每个字有4个词位：词首B、词中M、词尾E、单字成词S。基于CRF的分词方法与HMM不同，CRF是一种判别式模型
  > https://zhuanlan.zhihu.com/p/111899116
- 基于CRF的分词方法:分词当做字的词位分类问题，通常定义字的词位信息如下：词首常用B表示；词中常用M表示；词尾常用E表示；单子词，常用S表示。
